...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = b.legalNeighbors.toStream.map {
    case (nb, move) => ((nb, move :: history))
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    initial match {
      case (b, m) #:: xs => {
        val newNeighbors = newNeighborsOnly(neighborsWithHistory(b, m), explored)
        (b, m) #:: from(xs ++ newNeighbors, explored + b)
      }
      case _ => Stream.empty
    }
  }
